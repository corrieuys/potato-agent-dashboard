...
- Dashboard views:
  ...
  - Server instances ✅
  - Service status ✅
  - Event timeline ⏳ (heartbeats stored but no UI timeline view)
...
- Health checks ✅ (HTTP + process checks)
- External reverse proxy (path-based) ✅
- Internal DNS-based service discovery ✅ (via /etc/hosts)
- Optional firewall hardening ✅ (3 security modes via UFW)
- systemd integration ⏳ (mentioned but not implemented)
- Docker runtime support ✅ (bonus feature)
...
10. Dashboard UI ✅
11. Docker runtime support ✅ (bonus feature)

---

## 15.1 Bonus Features Implemented

The following features were added beyond the original MVP specification:

- **Docker Runtime**: Full Docker support including image building, container management, and image cleanup
- **Cloudflare Tunnel**: Automated tunnel creation and management for secure access
- **SSH Key Management**: Agent-side SSH key generation for private repository access
- **Health Check Server**: Built-in HTTP health check endpoint for monitoring
- **Memory Monitoring**: Process memory usage tracking in health checks
...
**Control Plane:**
- Full CRUD for stacks, services, and agents
- Agent registration with install tokens
- Desired state API with versioning and hashing
- Heartbeat ingestion and agent status tracking
- Web dashboard with HTMX for dynamic updates
- Full service configuration (git, build, run, env vars, health checks)

**Agent:**
- Registration and token exchange
- Desired state polling with change detection
- Git clone/pull with SSH key support for private repos
- Build command execution with output capture
- Process supervision with auto-restart on crashes
- Health check server (HTTP endpoint)
- External reverse proxy with path-based routing
- Internal proxy for service-to-service communication
- DNS entries via /etc/hosts (svc.internal domains)
- Docker runtime support (build and run containers)
- Cloudflare Tunnel integration
- Firewall management (3 security modes)
- SQLite state persistence

### ⏳ Pending Features

- **Event timeline UI**: Heartbeats are stored but no visual timeline
- **Git SHA resolution**: Plan mentions resolving branches to exact SHAs centrally
- **systemd integration**: Automatic service installation
- **Full DNS server**: Local DNS responder instead of /etc/hosts
- **WebSocket proxy support**: Mentioned but not explicitly implemented
- **Load balancer automation**: Multi-server setup mentioned but manual

## 17. What the MVP Looks Like
...
- Monitor everything centrally ✅ (basic monitoring via dashboard)
